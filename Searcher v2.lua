...
local function CreatePageBtn(num)
    ...
    b.BackgroundColor3 = (num == CurrentApiPage) and Color3.fromRGB(40,40,40) or C.Card
    b.Text = tostring(num)
    b.TextColor3 = (num == CurrentApiPage) and C.Accent or C.SubText
    ...
    b.MouseButton1Click:Connect(function()
        for _, btn in pairs(PageBtns) do 
            btn.TextColor3 = C.SubText
...
function NewSearch(q)
    ...
    for i, btn in pairs(PageBtns) do
        btn:Destroy()
    end
    PageBtns = {}
...
Scroll:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
    if Scroll.CanvasPosition.Y + Scroll.AbsoluteWindowSize.Y >= Scroll.AbsoluteCanvasSize.Y - 50 then
        if not IsLoading and SearchBox.Text ~= "" then
            ...
            if not PageBtns[CurrentApiPage] then
                PageBtns[CurrentApiPage] = CreatePageBtn(CurrentApiPage)
            end
            ...
            if PageBtns[CurrentApiPage] then
                PageBtns[CurrentApiPage].TextColor3 = C.Accent
                PageBtns[CurrentApiPage].BackgroundColor3 = Color3.fromRGB(40,40,40)
            end
